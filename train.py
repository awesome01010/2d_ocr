...
from modules.optimizer.uncertainly_loss import UncertainlyLoss
...
def train(opt):
    ...
    if opt.select_data == 'baidu':
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            collate_fn=BaiduCollate(opt.imgH, opt.imgW, keep_ratio=False, rotate=opt.rotate)
        )
    ...
    if opt.uncertainly_loss:
        weighted_loss_func = UncertainlyLoss(2)
        weighted_loss_func.cuda()
        for p in filter(lambda p: p.requires_grad, list(model.parameters())+list(weighted_loss_func.parameters())):
    ...
    while(True):
        ...
        if 'CTC' in opt.Prediction:
            ...
            # print("1", preds[0].shape)

            if opt.uncertainly_loss:
                cost = weighted_loss_func(criterion(preds[0].view(-1, preds[0].shape[-1]), text.contiguous().view(-1)),
        ...
        if i % opt.disInterval == 0:
            ...
            print(f'[{i}/{opt.num_iter}] Loss: {loss_avg.val():0.5f} elapsed_time: {elapsed_time:0.5f} lr: {now_lr:0.7f}')
            start_time = time.time()
        ...
        if i > 0 and i % (12*step_per_epoch) == 0:                # 调整学习速率
            lrScheduler.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--saveInterval', type=int, default=3000, help='Interval between each save')
    ...
    parser.add_argument('--uncertainly_loss', default=False, help='uncertainly loss')
    """ all baidu images """
    ...
    parser.add_argument('--baidu_alphabet', type=str, default='./dataset/alphabet.txt')

    
    parser.add_argument('--max_seq', type=int, default=40, help='the maxium of the sequence length')
    ...
    parser.add_argument('--rotate', action='store_true', help='picture rotate angle[  ]')
    """ Model Architecture """
